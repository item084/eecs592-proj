...
class Trainer(object):
    ...
    def list2sentence(self, ix):
        sent = []
        for i in ix:
            if i==0:
                continue
            sent.append(self.ixtoword[i])
        sent = '_'.join(sent)
        return sent
    ...
    def build_models(self):
        ...
        # text_encoder =  torch.hub.load('huggingface/pytorch-transformers', 'model', 'bert-base-uncased')  #en_trf_distilbertbaseuncased_lg
        ...
        caption_rnn = CAPTION_RNN(cfg.CAP.embed_size, cfg.CAP.hidden_size, self.n_words, cfg.CAP.num_layers)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                inputs = tokenizer(sentence_list, return_tensors="pt", padding=True)
                for k in inputs:
                    inputs[k] = inputs[k].cuda()

                emb = text_encoder(**inputs)
                words_embs = emb.last_hidden_state.permute(0, 2, 1)
                sent_emb = emb.pooler_output
                #sent_emb = words_embs.mean(dim=2)
            ...
            for _ in range(1):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    for j in range(batch_size):
                        ...
                        im.save(fullpath)
                        # print(keys[j], self.list2sentence(captions[j].tolist()))
